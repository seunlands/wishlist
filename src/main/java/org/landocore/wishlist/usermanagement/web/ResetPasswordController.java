...
public class ResetPasswordController implements Serializable {
	 ...
	 * the mail template.
	 */
	@ManagedProperty(value = "#{templateEmail}")
	...
	public final void resetPassword() {
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
				"Request received to reset password for user "
				+ username);
		}
		...
		if (user != null) {
			...
			String templateFilename = emailBundle.
				getString("email.password.reset.template");

			InputStream is = this.getClass().getClassLoader().
					getResourceAsStream(templateFilename);

			String body = LibelleUtil.getString(is,
				new Object[]{user.getUsername(),
				user.getPassword()});

			message.setText(body);
			//the subject
			String subject = emailBundle.
				getString("email.password.reset.subject");
	 ...
	 */
	public final void setResetPasswordForm(
			final ResetPasswordForm pResetPasswordForm) {
